...
const SingleProductViewer = () => {
  ...
  const singleProductRequiredFields = [
    "product_id",
    "name",
    "title",
    "size",
  ...
  useEffect(() => {
    ...
    if (storeProduct) {
      console.log("store single product = ", storeProduct);
      setProduct(storeProduct);
      setLoading(false);
      return;
    }

    // If we already have the product in state, don't fetch again
    if (product && product.product_id === Number(productId)) {
      setLoading(false);
      return;
    }
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        const res = await fetch(`${apiUrl}/api/v1/product/${productId}`, {
          ...
          body: JSON.stringify({
            requireField: singleProductRequiredFields,
          }),
        });

        if (res.ok && isMounted) {
          const data = await res.json();
          console.log(`single product data: ${JSON.stringify(data, null, 2)}`);
          setProduct(data.product || null);
        }
      } catch (err) {
        if (isMounted) console.error("Error fetching product:", err);
      } finally {
        if (isMounted) {
          setLoading(false);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 ">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {has3DModel && (
                <button
                  onClick={() => setActiveGalleryTab("3dmodel")}
                  className={`flex items-center gap-2 px-4 py-2 border rounded-lg transition-all duration-200 ${
            ...
            <div
              ...
              style={{
                ...
                borderColor: "var(--color-border)",
                minHeight: activeGalleryTab === "3dmodel" ? "500px" : "auto",
              }}
            >
              ...
              {/* Share Button */}
              <div className="absolute top-3 right-3 z-10" ref={popupRef}>
                <button
                ...
                {showSharePopup && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      ...
                      ].map(({ platform, icon: Icon, color, title }) => (
                        <button
                          key={platform}
            ...
            {activeGalleryTab === "images" && product.images?.length > 1 && (
              <div className="px-2">
                <Swiper
                  key="thumbs-swiper"
                  modules={[FreeMode, Navigation, Thumbs]}
